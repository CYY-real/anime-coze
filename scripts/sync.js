...
// ---- 6789kb 自动建映射：用 TMDB 剧名反查 vodId（maccms ajax 搜索接口）----
// 目的：新番只需写进 watchlist，CI 跑 sync.js 时自动用 TMDB 剧名去 6789kb 搜出 vodId，
//       写回 data/source-6789kb.json 缓存，后续直接复用，无需人工登记。
const KB_SEARCH_URL = 'https://www.6789kb.com/index.php/ajax/suggest?mid=1&wd=';
const KB_UA = {
  'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Safari/537.36',
  Referer: 'https://www.6789kb.com/',
};
// 软标记：命中后降低匹配优先级（仍是正片，如「第X季」「最终季」）
const SUB_MARK = /(外传|oad|剧场版|特别篇|总集|编年史|真人版|完结篇|前篇|后篇|最终季|最终|part|篇|季)/i;
// 硬排除：即使名字很像也绝不自动采用，避免兜底到 OAD/剧场版/外传等子条目
const HARD_BAD = /(外传|oad|剧场版|特别篇|总集篇|编年史|真人版|完结篇|前篇|后篇|part)/i;
...
function pickKbVod(tmdbName, cands) {
  ...
  for (const c of cands) {
    ...
    if (HARD_BAD.test(c.name)) continue; // 硬排除子条目（外传/OAD/剧场版等）绝不参与竞选
    if (score > bestScore) { bestScore = score; best = c; }
  }
  if (!best) return null;
  // 高分且非硬排除子条目 → 采用；或几乎精确匹配（>=90）即便偏门也采用
...
async function main() {
  ...
  let kbDirty = false; // 本次是否自动新匹配到映射，需写回仓库
  ...
  for (const id of pinnedIds) {
    ...
    if (!vodId) {
      // 映射缺失：用 TMDB 剧名反查 6789kb vodId，自动建映射并缓存
      await sleep(300); // 限流，避免触发站点风控
      ...
      if (picked) {
        ...
        kbDirty = true;
        console.log(`  ↳ 自动匹配 6789kb: ${preset.name} → vodId ${picked}`);
      } else {
        console.log(`  ↳ 6789kb 未匹配，沿用 TMDB: ${preset.name}`);
      }
    }
    if (vodId) {
  ...
  // 把本次自动新匹配的 6789kb 映射写回仓库，避免每次 CI 重复搜索（人工也可直接编辑此文件覆盖）
  if (kbDirty) {
    try {
      const kbPath = path.join(__dirname, '..', 'data', 'source-6789kb.json');
      fs.writeFileSync(kbPath, JSON.stringify(kbMap, null, 2));
      console.log(`💾 已缓存 ${Object.keys(kbMap).length} 条 6789kb 映射 → ${kbPath}`);
    } catch (e) {
      console.warn('⚠️ 写回 6789kb 映射失败（不影响本次同步）:', e.message);
    }
  }
